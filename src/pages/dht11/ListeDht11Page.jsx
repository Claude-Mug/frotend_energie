...
import EtatDht11Modal from './EtatDht11Page';
...
// --- Composant Principal : ListeDht11Page ---
function ListeDht11Page() {
  const navigate = useNavigate();
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['dht11Data'],
    queryFn: fetchDht11Data,
    staleTime: 5 * 60 * 1000,
  });
  ...
  const handleDelete = async (ids) => {
    try {
      ...
      await Promise.all(
        ids.map(id => axios.delete(`/api/dht11/${id}`))
      );
      toast.current.show({ severity: 'success', summary: 'Succès', detail: 'Capteur(s) DHT11 supprimé(s) avec succès', life: 3000 });
  ...
  const leftToolbarContent = () => (
    <div className="d-flex flex-wrap gap-2">
      <Button
        label="Ajouter"
        icon="pi pi-plus"
        className="p-button-secondary text-info p-button-sm"
      ...
      <Button
        ...
        onClick={() => navigate('/dht11/graphiques')}
      />
    ...
    <div className="py-2">
      ...
      <div className="card shadow-sm">
        <div className="card-header bg-white border-bottom-0">
          <h2 className="card-title h4 fw-bold mb-0">Liste des Capteurs DHT11</h2>
        </div>
        <div className="card-body">
          ...
          {isLoading ? (
            ...
            <div className="d-flex flex-column align-items-center justify-content-center p-4">
              ...
              <Button
                label="Ajouter un capteur DHT11"
                icon="pi pi-plus"
