...
import { fetchDht11Data } from './ListeDht11Page';
...
const GraphiqueDht11Page = () => {
  ...
  useEffect(() => {
    const getInitialData = async () => {
      setLoading(true);
      try {
        const rawData = await fetchDht11Data();
        setAllData(rawData);
        ...
        if (rawData.length > 0) {
          ...
          const defaultData = rawData.slice(-10);
  ...
  const handleDateChange = (value, setter) => {
    if (value) {
      // Vérifie si la date sélectionnée est dans le tableau des dates désactivées
  ...
  useEffect(() => {
      ...
      datasets: [
        {
          ...
          backgroundColor: filteredData.map(d => {
            ...
            if (temp < 15) return 'rgba(54, 162, 235, 0.8)';
            if (temp < 20) return 'rgba(75, 192, 192, 0.8)';
            if (temp < 25) return 'rgba(255, 205, 86, 0.8)';
            if (temp < 30) return 'rgba(255, 159, 64, 0.8)';
    ...
    const tempTooltipCallback = (context) => {
      const record = filteredData[context.dataIndex];
      return [
        `Date/Heure: ${moment(record.datetime).format('DD/MM/YYYY HH:mm')}`,
        `Température: ${record.temperature.toFixed(1)}°C`,
      ];
    };
    const tempOptions = chartOptions('Variation de température par enregistrement', 'Température (°C)', tempTooltipCallback);
    ...
    const humData = {
      ...
      datasets: [
        {
          label: 'Humidité',
          data: filteredData.map(d => d.humidite),
          backgroundColor: filteredData.map(d => {
            const hum = d.humidite;
            if (hum < 50) return 'rgba(153, 102, 255, 0.8)';
            if (hum < 70) return 'rgba(75, 192, 192, 0.8)';
    ...
    const humTooltipCallback = (context) => {
      const record = filteredData[context.dataIndex];
      return [
        `Date/Heure: ${moment(record.datetime).format('DD/MM/YYYY HH:mm')}`,
        `Humidité: ${record.humidite.toFixed(1)}%`,
      ];
    };
    const humOptions = chartOptions('Variation d\'humidité par enregistrement', 'Humidité (%)', humTooltipCallback);

    if (humCanvasRef.current) {
      humChartRef.current = new Chart(humCanvasRef.current, {
        type: 'bar',
        data: humData,
        options: humOptions,
      });
  ...
  const handleLast10Records = () => {
    if (allData.length === 0) return;
    const defaultData = allData.slice(-10);
    setFilteredData(defaultData);
  ...
  return (
    <div className="container py-4">
      <div className="text-center mb-4">
        <h1>Dashboard de Données DHT11</h1>
        <p className="lead">Visualisation de la température et de l'humidité</p>
      ...
      {filteredData.length > 0 && !loading && (
        <div className="row">
          <div className="col-12 col-lg-6 mb-4">
            <div className="card h-100 border-0 shadow-sm">
              <div className="card-header bg-primary text-white d-flex justify-content-between align-items-center border-0 rounded-top">
                <div>
                  <i className="pi pi-thermometer me-2"></i>
                  **Température (°C)**
                </div>
              </div>
          ...
          <div className="col-12 col-lg-6 mb-4">
            <div className="card h-100 border-0 shadow-sm">
              <div className="card-header bg-info text-white d-flex justify-content-between align-items-center border-0 rounded-top">
                <div>
                  <i className="pi pi-cloud me-2"></i>
                  **Humidité (%)**
                </div>
              </div>
              <div className="card-body" style={{ position: 'relative', height: '400px' }}>
...
export default GraphiqueDht11Page;