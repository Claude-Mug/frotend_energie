// src/pages/actionneurs/UpdateActionneursPage.jsx
...
// Le composant reçoit `visible`, `onHide`, et `data` (l'actionneur à modifier)
function UpdateActionneurModal({ visible, onHide, data, onRefresh }) {
  ...
  const updateActionneurMutation = useMutation({
    mutationFn: (updatedActionneur) => axios.put(`/api/actionneurs/${updatedActionneur.id}`, updatedActionneur),
    onSuccess: () => {
      toast.current.show({ severity: 'success', summary: 'Succès', detail: 'Actionneur mis à jour avec succès', life: 3000 });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    updateActionneurMutation.mutate(formData);
  };
