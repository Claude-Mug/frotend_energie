...
import { fetchLdrData } from './ListeLdrPage';
...
const GraphiqueLdrPage = () => {
  const luminositeCanvasRef = useRef(null);
  const luminositeChartRef = useRef(null);
  ...
  useEffect(() => {
    const getInitialData = async () => {
      setLoading(true);
      try {
        const rawData = await fetchLdrData();
        setAllData(rawData);
  ...
  useEffect(() => {
    if (luminositeChartRef.current) luminositeChartRef.current.destroy();
    ...
    const luminositeData = {
      labels: filteredData.map(d => moment(d.datetime).format('DD/MM HH:mm')),
      datasets: [
        {
          label: 'Luminosité',
          data: filteredData.map(d => d.luminosite_niveau),
          backgroundColor: filteredData.map(d => {
            const lum = d.luminosite_niveau;
            if (lum < 200) return 'rgba(255, 205, 86, 0.8)';
            if (lum < 500) return 'rgba(75, 192, 192, 0.8)';
    ...
    const luminositeTooltipCallback = (context) => {
      const record = filteredData[context.dataIndex];
      return [
        `Date/Heure: ${moment(record.datetime).format('DD/MM/YYYY HH:mm')}`,
        `Luminosité: ${record.luminosite_niveau}`,
      ];
    };
    const luminositeOptions = chartOptions('Variation de luminosité par enregistrement', 'Niveau de luminosité', luminositeTooltipCallback);
    
    if (luminositeCanvasRef.current) {
      luminositeChartRef.current = new Chart(luminositeCanvasRef.current, {
        type: 'bar',
        data: luminositeData,
        options: luminositeOptions,
      });
    }
  ...
  return (
    <div className="container py-4">
      <div className="text-center mb-4">
        <h1>Dashboard de Données LDR</h1>
        <p className="lead">Visualisation de la luminosité</p>
        <button
          className="btn btn-primary mt-3"
          onClick={() => navigate('/ldr')} // Route vers la table
        >
          ...
          <div className="col-12 mb-4">
            <div className="card h-100 border-0 shadow-sm">
              <div className="card-header bg-primary text-white d-flex justify-content-between align-items-center border-0 rounded-top">
                <div>
                  <i className="pi pi-sun me-2"></i>
                  **Luminosité**
                </div>
              </div>
              <div className="card-body" style={{ position: 'relative', height: '400px' }}>
                <canvas ref={luminositeCanvasRef}></canvas>
              </div>
            </div>
          </div>
        </div>
...
export default GraphiqueLdrPage;